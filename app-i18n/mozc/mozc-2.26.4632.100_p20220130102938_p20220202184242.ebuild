...
DEPEND=">=dev-cpp/abseil-cpp-20210324
	>=dev-libs/protobuf-3.0.0:=
	fcitx4? (
		app-i18n/fcitx:4
		virtual/libintl
...
RDEPEND=">=dev-cpp/abseil-cpp-20210324
	>=dev-libs/protobuf-3.0.0:=
	emacs? ( app-editors/emacs:* )
	fcitx4? (
		app-i18n/fcitx:4
...
src_install() {
	...
	if use fcitx5; then
		...
		execute msgfmt --xml -d unix/fcitx5/po --template unix/fcitx5/org.fcitx.Fcitx5.Addon.Mozc.metainfo.xml.in -o unix/fcitx5/org.fcitx.Fcitx5.Addon.Mozc.metainfo.xml
		insinto /usr/share/metainfo
		doins unix/fcitx5/org.fcitx.Fcitx5.Addon.Mozc.metainfo.xml
		execute rm -f unix/fcitx5/org.fcitx.Fcitx5.Addon.Mozc.metainfo.xml

		insinto /usr/share/icons/hicolor/128x128/apps
		newins data/images/product_icon_32bpp-128.png org.fcitx.Fcitx5.fcitx-mozc.png
		dosym org.fcitx.Fcitx5.fcitx-mozc.png /usr/share/icons/hicolor/128x128/apps/fcitx-mozc.png

		insinto /usr/share/icons/hicolor/32x32/apps
		newins data/images/unix/ime_product_icon_opensource-32.png org.fcitx.Fcitx5.fcitx-mozc.png
		dosym org.fcitx.Fcitx5.fcitx-mozc.png /usr/share/icons/hicolor/32x32/apps/fcitx-mozc.png

		local icon_dir image image_name
		icon_dir="/usr/share/icons/hicolor/48x48/apps"
		insinto "${icon_dir}"
		for image in ../../fcitx-${PN}/src/data/images/unix/ui-*.png; do
			image_name="${image#../../fcitx-${PN}/src/data/images/unix/ui-}"
			newins "${image}" "org.fcitx.Fcitx5.fcitx-mozc-${image_name}"
			dosym "org.fcitx.Fcitx5.fcitx-mozc-${image_name}" "${icon_dir}/fcitx-mozc-${image_name}"
		done

		local po_file mo_file locale
		for po_file in unix/fcitx5/po/*.po; do
			locale="${po_file##*/}"
			locale="${locale%.po}"
			mo_file="${po_file/.po/.mo}"
			execute msgfmt $po_file -o $mo_file
			insinto /usr/share/locale/${locale}/LC_MESSAGES
			newins "${mo_file}" fcitx5-mozc.mo
			execute rm -f $mo_file
		done
	fi
